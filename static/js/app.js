...
window.addEventListener('DOMContentLoaded', async function() {
    try {
        await loadSettings();  // 先加载配�?        await initUser();
        await loadStats();
        await loadImage();
    } catch (e) {
...
async function initUser() {
    try {
        ...
        let response;
        if (!userId) {
            // 创建新用�?            response = await fetch('/api/user/init');
...
async function loadStats() {
    try {
        ...
        const requiredVotes = totalImageCount * 5;  // 每张图需�?�?        const totalVotes = stats.total_reviews || 0;
        ...
        // 显示完成审核的图片数（获�?票且�?通过�?        if (completeCount) completeCount.textContent = stats.completed_images || 0;
        if (totalImages) totalImages.textContent = totalImageCount;
        
        // 更新用户审核�?        if (currentUser) {
...
async function loadImage() {
    ...
    if (!currentUser || !currentUser.id) {
        if (loading) loading.textContent = '等待初始�?..';
...
async function submitReview(status) {
    ...
    try {
        ...
        // 保存到历�?        historyStack.push(currentImage);
...
function showImageDetail() {
    ...
    if (currentImage.is_reviewed_by_user) {
        const statusMap = { pass: '已通过', fail: '未通过', skip: '已跳�? };
        if (myStatus) {
...
async function showRuleModal() {
    ...
    if (content) content.innerHTML = '<p style="color:#888;">加载�?..</p>';
...
// 简单的Markdown解析（带XSS防护�?function parseMarkdown(text) {
    if (!text) return '';
    
    // 第一步：HTML实体转义（防止XSS�?    let escaped = text
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
        .replace(/"/g, '&quot;')
    ...
    // 第二步：解析Markdown语法
    return escaped
        // 标题（使用捕获组避免XSS�?        .replace(/^### (.+)$/gm, '<h3>$1</h3>')
        .replace(/^## (.+)$/gm, '<h2>$1</h2>')
        .replace(/^# (.+)$/gm, '<h1>$1</h1>')
